...
public class BigBedToBed {
  ...
  public static void main(final Path inputPath, final Path outputPath,
                          final String queryChromName, final int queryStart,
                          final int queryEnd, final int maxItems)
      throws IOException {
    try (BigBedFile bf = BigBedFile.parse(inputPath);
         BufferedWriter out = Files.newBufferedWriter(outputPath)) {
      int itemCount = 0;
      for (final String chromName : bf.chromosomes()) {
        if (!queryChromName.isEmpty() && !chromName.equals(queryChromName)) {
          continue;
        }
        ...
        for (final BedData interval : bf.query(chromName, queryStart, queryEnd, itemsLeft)) {
